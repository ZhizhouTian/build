                                   ...
                                   const uint8_t       *data)
{
...
#ifdef IK_64BITS
  ScannedMember *first_member_slab = kmalloc(sizeof(*first_member_slab)*(1<<FIRST_SCANNED_MEMBER_SLAB_SIZE_LOG2), GFP_KERNEL);
#else
  ...
  /* scanned_member_slabs[i] is an array of arrays of ScannedMember.
     ...
     using the allocator. */
#ifdef IK_64BITS
  ScannedMember **scanned_member_slabs = kmalloc(sizeof(*scanned_member_slabs)*(MAX_SCANNED_MEMBER_SLAB+1), GFP_KERNEL);
#else
...
#ifdef __KERNEL__
  required_fields_bitmap = kmalloc(required_fields_bitmap_len * sizeof(long), GFP_KERNEL);
#else
...
#ifdef IK_64BITS
  kfree(first_member_slab);
  kfree(scanned_member_slabs);
#endif
if (required_fields_bitmap)
#ifdef __KERNEL__
    kfree (required_fields_bitmap);
#endif
