...
static void node_edge_swap (void *l, void *r, int size)
{
	struct act_edge *le = l;
	struct act_edge *re = r;
	struct act_edge tmp;
...
{
	sort ((void *)nod->outgoing, nod->outgoing_size,
			sizeof(struct act_edge),
			node_edge_compare,
			node_edge_swap);
}
...
static void node_grow_outgoing_vector (ACT_NODE_t *thiz)
{
	...
	if (thiz->outgoing_capacity == 0)
	{
		...
		thiz->outgoing = (struct act_edge *) kmalloc
			(thiz->outgoing_capacity * sizeof(struct act_edge), GFP_KERNEL);
	}
	else
	{
		size_t new_cap = thiz->outgoing_capacity + grow_factor;
		struct act_edge *np = kmalloc(new_cap * sizeof(struct act_edge), GFP_KERNEL);

		if (np) {
			memcpy(np, thiz->outgoing, thiz->outgoing_capacity * sizeof(struct act_edge));
			kfree(thiz->outgoing);

			thiz->outgoing_capacity = new_cap;
			thiz->outgoing = np;
		} else {
...
static void node_grow_matched_vector (ACT_NODE_t *thiz)
{
	if (thiz->matched_capacity == 0)
	{
		...
		thiz->matched = (AC_PATTERN_t *) kmalloc
			(thiz->matched_capacity * sizeof(AC_PATTERN_t), GFP_KERNEL);
	}
	else
	{
		size_t new_cap = thiz->matched_capacity + 2;
		AC_PATTERN_t *np = kmalloc(new_cap * sizeof(AC_PATTERN_t), GFP_KERNEL);

		if (np) {
			memcpy(np, thiz->matched, thiz->matched_capacity * sizeof(AC_PATTERN_t));
			kfree(thiz->matched);

			thiz->matched_capacity = new_cap;
			thiz->matched = np;
		} else {
			pr_err("AC fail to allocate mem\n");
		}
		...
		for (j = 0; j < nod->matched_size; j++)
		{
			...
			switch (patt.id.type)
			{
				...
				case AC_PATTID_TYPE_NUMBER:
					pr_info("%ld", patt.id.u.number);
					break;
				case AC_PATTID_TYPE_STRING:
					pr_info("%s", patt.id.u.stringy);
					break;
			}
			pr_info(": %.*s", (int)patt.ptext.length, patt.ptext.astring);
		}
		pr_info("}\n");
	}
	pr_info("\n");
