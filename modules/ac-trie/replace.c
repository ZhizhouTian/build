...
void mf_repdata_allocbuf (MF_REPLACEMENT_DATA_t *rd)
{
	...
	if (rd->has_replacement)
	{
		rd->buffer.astring = (AC_ALPHABET_t *)
			kmalloc(MF_REPLACEMENT_BUFFER_SIZE * sizeof(AC_ALPHABET_t), GFP_KERNEL);

		rd->backlog.astring = (AC_ALPHABET_t *)
			kmalloc(AC_PATTRN_MAX_LENGTH * sizeof(AC_ALPHABET_t), GFP_KERNEL);
...
void mf_repdata_release (MF_REPLACEMENT_DATA_t *rd)
{
	kfree((AC_ALPHABET_t *)rd->buffer.astring);
	kfree((AC_ALPHABET_t *)rd->backlog.astring);
	kfree(rd->noms);
}
...
static void mf_repdata_grow_noms_array (MF_REPLACEMENT_DATA_t *rd)
{
	...
	if (rd->noms_capacity == 0)
	{
		...
		rd->noms = (struct mf_replacement_nominee *) kmalloc
			(rd->noms_capacity * sizeof(struct mf_replacement_nominee), GFP_KERNEL);
		rd->noms_size = 0;
	}
	else
	{

		size_t new_cap = rd->noms_capacity + grow_factor;
		struct mf_replacement_nominee *np = kmalloc(new_cap * sizeof(struct mf_replacement_nominee), GFP_KERNEL);

		if (np) {
			memcpy(np, rd->noms, rd->noms_capacity * sizeof(struct mf_replacement_nominee));
			kfree(rd->noms);

			rd->noms_capacity = new_cap;
			rd->noms = np;
		} else {
			pr_err("AC fail to allocate mem\n");
		}
	}
}
 ...
 * @param new_nom
 *****************************************************************************/
static void mf_repdata_push_nominee (MF_REPLACEMENT_DATA_t *rd, struct mf_replacement_nominee *new_nom)
{
 ...
 * @param to
 *****************************************************************************/
static void mf_repdata_appendfactor (MF_REPLACEMENT_DATA_t *rd, size_t from, size_t to)
{
 ...
 * @param to_position
 *****************************************************************************/
static void mf_repdata_do_replace (MF_REPLACEMENT_DATA_t *rd, size_t to_position)
{
	unsigned int index;
		...
		MF_REPLACE_MODE_t mode, MF_REPLACE_CALBACK_f callback, void *param)
{
	...
	size_t position_r = 0;  /* Relative curr position in the input string */
	...
	{
		if (!(next = node_find_next_bs(curr, instr->astring[position_r])))
		{
		...
		if (curr->final && next)
		{
			...
			nom.pattern = curr->to_be_replaced;
	...
	backlog_pos = thiz->base_position + instr->length - curr->depth;
