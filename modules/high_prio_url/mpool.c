...
static struct mpool_block *mpool_new_block (size_t size)
{
	...
	block = (struct mpool_block *) kmalloc(sizeof(struct mpool_block), GFP_KERNEL);

	block->bp = block->free = kmalloc(size, GFP_KERNEL);
	block->size = size;
	block->next = NULL;
...
struct mpool *mpool_create (size_t size)
{
	...
	ret = kmalloc(sizeof(struct mpool), GFP_KERNEL);
