...
{
    AC_TRIE_t *thiz = (AC_TRIE_t *) kmalloc (sizeof(AC_TRIE_t), GFP_KERNEL);
 ...
 *****************************************************************************/
AC_STATUS_t ac_trie_add (AC_TRIE_t *thiz, AC_PATTERN_t *patt, int copy, AC_PATTID_t **id)
{
    size_t i;
    ...
    if (patt->ptext.length > AC_PATTRN_MAX_LENGTH) {
        pr_err("AC: Pattern(%s) length(%u) is too large, reset it as %d\n",
              patt->ptext.astring, (int)patt->ptext.length, AC_PATTRN_MAX_LENGTH);
        patt->ptext.length = AC_PATTRN_MAX_LENGTH;
    }
        ...
        AC_MATCH_CALBACK_f callback, void *user)
{
    ...
    {
        if (!(next = node_find_next_bs (curr, text->astring[position])))
        {
         ...
         * report match, because it has already been reported */
        {
            ...
            match.size = curr->matched_size;
            match.patterns = curr->matched;
