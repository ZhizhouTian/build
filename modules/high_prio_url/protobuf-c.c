...
#define IK_MALLOC(size) kmalloc(size, GFP_ATOMIC)
#define IK_FREE(ptr) kfree(ptr)
#endif
                                   ...
                                   const uint8_t       *data)
{
...
#ifdef IK_64BITS
  ScannedMember *first_member_slab = IK_MALLOC(sizeof(*first_member_slab)*(1<<FIRST_SCANNED_MEMBER_SLAB_SIZE_LOG2));
#else
  ...
  /* scanned_member_slabs[i] is an array of arrays of ScannedMember.
     ...
     using the allocator. */
#ifdef IK_64BITS
  ScannedMember **scanned_member_slabs = IK_MALLOC(sizeof(*scanned_member_slabs)*(MAX_SCANNED_MEMBER_SLAB+1));
#else
...
#ifdef __KERNEL__
  required_fields_bitmap = IK_MALLOC(required_fields_bitmap_len * sizeof(long));
#else
...
#ifdef IK_64BITS
  IK_FREE(first_member_slab);
  IK_FREE(scanned_member_slabs);
#endif
if (required_fields_bitmap)
#ifdef __KERNEL__
    IK_FREE (required_fields_bitmap);
#endif
